...
ecr-login() {
  ...
  aws ecr get-login-password --profile "$profile" --region "$region" | docker login --username AWS --password-stdin "$(aws sts get-caller-identity --profile "$profile" --query 'Account' --output text).$endpoint_suffix.$region.amazonaws.com"
}
